...
fs.readFile("./3/input.txt", (err, data) => {
    ...
    const properExpressions = [...fileString.matchAll(correctlyFormedMulRegex)]
        .map((expr) => [Number(expr[1]), Number(expr[2])]).filter((factors) =>
            factors[0] < 1000 && factors[1] < 1000
        );
    const sumOfAllMuls = properExpressions.map((factors) =>
        factors[0] * factors[1]
    ).reduce((a, b) => a + b, 0);
