...
fs.readFile("./7/input.txt", (err, data)=> {
    ...
    const computationsInAssignment: Computation[] = [...data.toString().matchAll(COMPUTATION_LINE_REGEX)]
            .map((match)=>{
                if(!match){
    ...
    const correctComputation = computationsInAssignment.filter(c=>computationPossible(c))

    const totalCalibrationResult = correctComputation.reduce((n, c)=>n+c.result,0) 
    console.log(`The total calibration result (respecting the concatenation operator) is: ${totalCalibrationResult}`);
});
...
function computationPossible(c: Computation): boolean {
    ...
    if(last_element == undefined){
        console.error("ALGO ERROR"); return false;
    }
    ...
    return computationPossible(add_ver) || computationPossible(multiply_ver)  || computationPossible(concat_ver);
