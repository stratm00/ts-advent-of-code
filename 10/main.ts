...
fs.readFile("./10/input.txt", (err, data)=> {
    ...
    const _getSummitsFrom = (pos: Vec2n): Vec2n[] => {
        ...
        if( _getNumOnGrid(posRight) == posValue+STEPLENGTH)_listUnion(ourSummits, _getSummitsFrom(posRight))
        if( _getNumOnGrid(posLeft)  == posValue+STEPLENGTH)_listUnion(ourSummits, _getSummitsFrom(posLeft))
        if( _getNumOnGrid(posUp)    == posValue+STEPLENGTH)_listUnion(ourSummits, _getSummitsFrom(posUp));
        if( _getNumOnGrid(posDown)  == posValue+STEPLENGTH)_listUnion(ourSummits, _getSummitsFrom(posDown))
    ...
    const trailheads: Vec2n[] = [...bareData.matchAll(ZERO_REGEX)].map((regexarr)=>{
        const xOffset = regexarr.index % gridWidth;
        return {x: xOffset, y: (regexarr.index - xOffset) / gridWidth};
    });

    const successfulTrailsforTrailheads = trailheads.map(_getSummitsFrom).map(_getUniqueList);
