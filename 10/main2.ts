...
fs.readFile("./10/input.txt", (err, data)=> {
    ...
    const _getTrailheadScore = (pos: Vec2n): number => {
        ...
        if( _getNumOnGrid(posRight) == posValue+STEPLENGTH)sumOfDirs+=_getTrailheadScore(posRight);
        if( _getNumOnGrid(posLeft)  == posValue+STEPLENGTH)sumOfDirs+=_getTrailheadScore(posLeft);
        if( _getNumOnGrid(posUp)    == posValue+STEPLENGTH)sumOfDirs+=_getTrailheadScore(posUp);
        if( _getNumOnGrid(posDown)  == posValue+STEPLENGTH)sumOfDirs+=_getTrailheadScore(posDown);
