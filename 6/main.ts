...
fs.readFile("./6/input.txt", (err, data) => {
    ...
    const visitedPlaces = map.map((row) =>
        [...row.matchAll(MARKED_REGEX)].length
    )
        .reduce((a, b) => a + b, 0);
...
function completeWalkaround(map: Map): void {
    ...
    const _checkSolidAhead = (pos: Vec2n, orientation: Vec2n): boolean => {
        ...
        if (
            !((pos.x + orientation.x < mapWidth &&
                pos.x + orientation.x >= 0) &&
                (pos.y + orientation.y < mapHeight &&
                    pos.y + orientation.y >= 0))
        ) return false;
        return map[pos.y + orientation.y].charAt(pos.x + orientation.x) ===
            ROADBLOCK;
    };
