...
function add_vec2n(a: Vec2n, b: Vec2n): Vec2n {
...
fs.readFile("./6/input.txt", (err, data) => {
    ...
    for (let i = 0; i < mapHeight * mapWidth; i++) {
        ...
        if (_isFree(localMap, pointCoords)) {
            //Place Obstacle
            localMap[pointCoords.y] =
                localMap[pointCoords.y].substring(0, pointCoords.x) + "#" +
                localMap[pointCoords.y].substring(pointCoords.x + 1);
...
function completeWalkaround(map: Map): boolean {
    ...
    const _checkSolidAhead = (pos: Vec2n, orientation: Vec2n): boolean => {
        ...
        if (
            !((new_pos.x < mapWidth && new_pos.x >= 0) &&
                (new_pos.y < mapHeight && new_pos.y >= 0))
        ) return false;
    ...
    let guardHasNotLeftMap = (guard.x < mapWidth && guard.x >= 0) &&
        (guard.y < mapHeight && guard.y >= 0);
    ...
    while (guardHasNotLeftMap && !guardIsCaughtInLoop) {
        ...
        map[guard.y] = map[guard.y].substring(0, guard.x) + MARKED_CHAR +
        ...
        while (_checkSolidAhead(guard, orientation) && !guardIsCaughtInLoop) {
            ...
            const alreadyTurnedHere = guardTurningPoints.filter((info) =>
                (info.pos.x == guard.x && info.pos.y == guard.y) &&
                (info.dir.x == orientation.x && info.dir.y == orientation.y)
            ).length > 0;
            if (alreadyTurnedHere) {
                guardIsCaughtInLoop = true;
            }
            //Naive Solution
        ...
        if (!guardIsCaughtInLoop) {
            ...
            guardHasNotLeftMap = (guard.x < mapWidth && guard.x >= 0) &&
                (guard.y < mapHeight && guard.y >= 0);
