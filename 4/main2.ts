...
function sub_vec2n(x: Vec2n, sub:Vec2n) {return {x:x.x-sub.x, y:x.y-sub.y}}
function add_vec2n(x: Vec2n, sub:Vec2n) {return {x:x.x+sub.x, y:x.y+sub.y}}
    ...
    for(let y = 0; y<grid.length; y++){
        ...
        for(let x = 0;x<line.length;x++){
            ...
            if(line.charAt(x)==="A"){
                ...
                const diag_1_condition = _spellsOut(diag1_start, diag1, "SAM") || _spellsOut(diag1_start, diag1, "MAS");
                const diag_2_condition = _spellsOut(diag2_start, diag2, "SAM") || _spellsOut(diag2_start, diag2, "MAS");  
