...
fs.readFile("./2/input.txt", (err, data) => {
    ...
    const numberOfSafeReports = reports.filter((rep) =>
        isReportSafe(rep)
    ).length;
...
function isReportSafe(report: ReportData): boolean {
    ...
    for (const index in report) {
        ...
        if (num_index < report.length - 1) {
            ...
            action_in_acceptable_deltas = action_in_acceptable_deltas &&
                ((abs_delta >= ACCEPTABLE_DELTA_BOUNDS[0]) &&
                    (abs_delta <= ACCEPTABLE_DELTA_BOUNDS[1]));
        }
    }

    return ((monotonous_rising_action || monotonous_falling_action) &&
        action_in_acceptable_deltas);
}
