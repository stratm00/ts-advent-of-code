...
fs.readFile("./9/input.txt", (err, data) => {
    ...
    const readInstructions: InstructionData = data.toString().split("").map(
        Number,
    );
...
function compactDisk(d: Disk): Disk {
    ...
    while (consideredNumbers.length != allNumbers.length) {
        ...
        for (let i = 0; i < disk.length; i++) {
            ...
            if (
                disk[i] == EMPTY_SPACE_IDENTIFIER &&
                !consideredEmptySpaces.find((nf) =>
                    nf.index <= i && nf.index + nf.width > i
                )
            ) {
                let _width = 1;
                while (disk[i + _width] == disk[i]) _width++;
                consideredEmptySpaces.push({
                    number: -1,
                    index: i,
                    width: _width,
                });
        ...
        for (let i = disk.length - 1; i >= 0; i--) {
            if (
                disk[i] != EMPTY_SPACE_IDENTIFIER &&
                !consideredNumbers.includes(disk[i])
            ) {
                //we have a hit
                lastNumber = disk[i];
                numberWidth = 1;
        ...
        const emptySpaceFit = consideredEmptySpaces.find((nf) =>
            nf.width >= numberWidth && nf.index < lastNumberIndex
        );
...
function getAllDiskNumbers(d: Disk) {
    ...
    d.forEach((n) => {
        if (!(n == EMPTY_SPACE_IDENTIFIER || baseList.includes(n))) {
            baseList.push(n);
        }
    });
    return baseList;
...
function getDiskChecksum(d: Disk): number {
    return d.map((value: number, index: number) => {
        return value != EMPTY_SPACE_IDENTIFIER ? value * index : 0;
    }).reduce((a, b) => a + b, 0);
}
