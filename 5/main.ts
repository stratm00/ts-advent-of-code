...
fs.readFile("./5/input.txt", (err, data)=> {
    ...
    const printStatementsString = dataString.substring(splitIndex+3,dataString.length);
    
    const rulesList: RuleStatement[] = [...ruleStatementsString.matchAll(rule_regex)]
                                    .map((match)=>{
    ...
    const printedPageOrders = printOrdersList.map((order)=>getPrintableOrder(order, rulesList)).filter((x)=>x!=undefined);
    
    const sumOfMiddlePages = printedPageOrders.map((order)=>order[Math.floor(order.length/2)]).reduce((a,b)=>a+b, 0);
    console.log(`Sum of middle pages: ${sumOfMiddlePages}`);
})

function getPrintableOrder(order:PrintOrder, rules:RuleStatement[]): number[] | undefined{
