...
fs.readFile("./5/input.txt", (err, data)=> {
    ...
    const problemOrders = printOrdersList.filter(order=>orderViolatesRules(order, rulesList));
...
function _getApplicableRules(order:PrintOrder, rules:RuleStatement[]):RuleStatement[]{
