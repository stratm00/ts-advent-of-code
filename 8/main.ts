...
fs.readFile("./8/input.txt", (err, data) => {
        ...
        return [
            {
                ...
                point: vec2n_add(s2.point, vectorDiff),
            },
            {
                idx: -1,
                identifier: "#" + s2.identifier,
                point: vec2n_sub(s1.point, vectorDiff),
            },
        ];
    };
    ...
    for (const hit of stationHits) {
        ...
        for (const stationAhead of siblingStationsAhead) {
            ...
            if (
                !totalAntinodes.find((an) =>
                    JSON.stringify(an) == JSON.stringify(antinodes[0])
                )
            ) totalAntinodes.push(antinodes[0]);
            if (
                !totalAntinodes.find((an) =>
                    JSON.stringify(an) == JSON.stringify(antinodes[1])
                )
            ) totalAntinodes.push(antinodes[1]);
        }
    }
    ...
    const antinodesInBounds = totalAntinodes.map((an) => an.point).filter(
        _isInBounds,
    );
    ...
    console.log(
        `There are ${uniqueLocations.length} total unique locations of antinodes on map`,
    );
});
